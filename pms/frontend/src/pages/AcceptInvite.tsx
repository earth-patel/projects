...
const AcceptInvite = () => {
  ...
  const { invitationInfo, invitationLoading, invitationInfoError } = useAppSelector(state => state.invitation);
  ...
  if (invitationInfoError) {
    return (
      <div>
        ...
        <p>{invitationInfoError.errors?.general || 'This invitation is invalid or has expired.'}</p>
  ...
  return (
    <div className="page-center">
      <div className="content-card content-card-sm content-card-centered">
        ...
        <div className="mb-3">
          <div className="title">
            Join <strong>{invitationInfo.organization.name}</strong> as a <strong>{invitationInfo.role.name}</strong>.
          </div>

          {!user && (<div className="subtitle mt-1">
